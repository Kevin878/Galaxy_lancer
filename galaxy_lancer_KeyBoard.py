...
def bring_enemy():
    ...
    # 自動化出現敵機
    sec = tmr/FPS
    if tmr % int(FPS / (mode_number + 2)) == 0:
        if 0 < sec < 15:
    ...
    if tmr == 65*FPS:
        set_enemy(random.randint(20, SCREEN_X-20), LINE_T, 90, ENEMY_BOSS, 10+mode_number, 100+mode_number*50)
    ...
    for id in range(ENEMY_MAX):
        if emy_f[id]:
            if emy_type[id] not in (ENEMY_BOSS, ENEMY_BOSS+1): #管理普通敵機的移動
                ...
                if emy_shield[id] < (100+mode_number*50)*0.25 and dis == 0: #當血量只剩下四分之一時
            ...
            if emy_type[id] in (ENEMY_BOSS, ENEMY_BOSS+1):
                boss_max_shield = 100 + 50 * mode_number
                ...
                pygame.draw.rect(scrn, (64, 32, 32), [
                    emy_x[id] - img_shield.get_width() // 2 + (emy_shield[id] / boss_max_shield)*400,
                    emy_y[id] - img_enemy[emy_type[id]].get_height() // 4 - img_shield.get_height(),
                    (boss_max_shield - emy_shield[id]) / boss_max_shield * 400,
                    12
                ])
...
def main(): # 主要迴圈
    ...
    while True:
        ...
        menu_up = keys[K_UP] or keys[K_w]
        menu_down = keys[K_DOWN] or keys[K_s]
        menu_confirm = keys[K_RETURN] or keys[K_SPACE]
        axis0[0] = 0
        axis1[0] = 0
        ...
        BTleft[0] = 1 if (keys[K_LEFT] or keys[K_a]) else 0
        BTright[0] = 1 if keys[K_SPACE] else 0
        BTup[0] = 0
        BTdown[0] = 1 if keys[K_z] else 0

        if idx == -1:
            if tmr == 1:
                speed = 5

            Key_Down = (Key_Down+1) * menu_down
            Key_Up = (Key_Up+1) * menu_up
        ...
        if idx == 2:
            ...
            if tmr <= 90:
                if tmr%5 == 0:
                    set_effect(ss_x[0]+random.randint(-60, 60), ss_y[0]+random.randint(-60, 60))
                if tmr%10 == 0:
                    se_damage.play()
            if tmr == 120:
